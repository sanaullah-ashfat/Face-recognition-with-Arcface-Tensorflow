...
def prepare_facebank(cfg, model):
    ...
    embeddings=[]
    detector = MTCNN()
    for name in os.listdir(cfg['image']):
        if os.path.isfile(name):
            continue
        else:
            emb = []
            for file in tqdm(os.listdir(os.path.join(cfg['image'], name))):
                if not os.path.isfile(os.path.join(cfg['image'], name, file)):
                    continue
                else:
                    image = cv2.imread(os.path.join(cfg['image'], name, file))
                   # print(image)
    ...
    with open("/home/cspd/Documents/project_integrate/embds_dict.pkl", 'rb') as file:
        file=pickle.load(file)
    
    return file.update(embds_dict)
...
embd = prepare_facebank(cfg, model)

pickle.dump(embd, open("/home/cspd/Documents/face_recognition/embds_dict_ad.pkl", 'wb'))
